...
** A C++ header-only library for wrapping ZLib streams into Standard Library streams

This library enables the use of C++ standard iostreams to access ZLib-compressed streams.

For input access, compression format is auto-detected, and multiple concatenated streams are decompressed samelessly.
...
For an example usage, see [[examples/zpipe.cpp]].
